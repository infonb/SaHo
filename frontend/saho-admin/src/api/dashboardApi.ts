...
export const getDashboardStats = async (): Promise<DashboardStats> => {
  ...
  return {
    ...
    total_sponsors: sponsors.length, active_sponsors: sponsors.length, total_pledged: sponsors.reduce((t, s) => t + Number(s.contrib_amt), 0), total_volunteers: volunteers.length, active_volunteers: volunteers.length, total_admins: admins.length,
    ...
    top_sponsors: [...sponsors].sort((a, b) => b.contrib_amt - a.contrib_amt).slice(0, 4).map(buildSponsorView),
  };
};
