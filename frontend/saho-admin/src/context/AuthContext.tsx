...
interface AuthContextValue { user: AuthUser | null; login: (user: AuthUser, token: string) => void; logout: () => void; isAuthenticated: boolean; }
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const login = (u: AuthUser, token: string) => { localStorage.setItem('saho_user', JSON.stringify(u)); localStorage.setItem('saho_token', token); setUser(u); };
