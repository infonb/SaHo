...
export default function StudentFormPage({ embedded = false, onCancel, onSuccess }: StudentFormPageProps) {
  ...
  useEffect(() => {
    const loadMasterData = async () => {
      ...
      try {
        ...
        setStudentStateList(statesResponse.map((item) => ({ stId: Number(item.stId ?? item.st_id), stName: String(item.stName ?? item.st_name ?? '') })));
        setStates(statesResponse.map((item) => [String(item.stId ?? item.st_id), String(item.stName ?? item.st_name ?? '')]));
        setCastes(castesResponse.map((item) => [String(item.casteId), String(item.casteName ?? '')]));
        setRelationships(relationshipsResponse.map((item) => [String(item.relationship_id), String(item.relationship_name ?? '')]));
        setClasses(classesResponse.map((item) => [String(item.classId), String(item.className ?? '')]));
  ...
  const loadDistricts = async (stateId: number) => {
    ...
    try {
      ...
      setDistricts(response.map((item) => [String(item.distId ?? item.dist_id), String(item.distName ?? item.dist_name ?? '')]));
    } catch {
  ...
  const loadMandals = async (districtId: number) => {
    ...
    try {
      ...
      setMandals(response.map((item) => [String(item.mndlId ?? item.mndl_id), String(item.mndlName ?? item.mndl_name ?? '')]));
    } catch {
  ...
  const loadVillages = async (mandalId: number) => {
    ...
    try {
      ...
      setVillages(response.map((item) => [String(item.vilId ?? item.vil_id), String(item.vilName ?? item.vil_name ?? '')]));
    } catch {
  ...
  const loadSchools = async (villageId: number) => {
    ...
    try {
      ...
      setSchools(response.map((item) => [String(item.schId ?? item.sch_id), String(item.schName ?? item.sch_name ?? '')]));
    } catch {
  ...
  const loadStudent = async (studentId: number, loadedStates: { stId?: number; st_id?: number; stName?: string; st_name?: string }[]) => {
    setEditLoading(true);
    try {
      ...
      if (student.stName) {
        const state = loadedStates.find((item) => (item.stName ?? item.st_name) === student.stName);
        if (state) {
          const stateId = Number(state.stId ?? state.st_id ?? 0);
          setForm((prev) => ({ ...prev, st_id: String(stateId) }));
          const districtsResponse = await getDistrictsByState(stateId);
          districtEntries = districtsResponse.map((item) => [String(item.distId ?? item.dist_id), String(item.distName ?? item.dist_name ?? '')]);
      ...
      if (student.distName && districtEntries.length) {
        ...
        if (dist) {
          ...
          mandalEntries = mandalsResponse.map((item) => [String(item.mndlId ?? item.mndl_id), String(item.mndlName ?? item.mndl_name ?? '')]);
      ...
      if (student.mndlName && mandalEntries.length) {
        ...
        if (mandal) {
          ...
          villageEntries = villagesResponse.map((item) => [String(item.vilId ?? item.vil_id), String(item.vilName ?? item.vil_name ?? '')]);
      ...
      if (student.vilName && villageEntries.length) {
        ...
        if (village) {
          ...
          schoolEntries = schoolsResponse.map((item) => [String(item.schId ?? item.sch_id), String(item.schName ?? item.sch_name ?? '')]);
