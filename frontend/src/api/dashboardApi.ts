...
export const getDashboardStats = async (): Promise<DashboardStats> => {
  ...
  return {
    ...
    total_sponsors: sponsors.length, active_sponsors: sponsors.length, total_pledged: sponsors.reduce((t, s) => t + numericContribution(s.contrib), 0), total_volunteers: volunteers.length, active_volunteers: volunteers.length, total_admins: admins.length,
    ...
    top_sponsors: [...sponsors].sort((a, b) => numericContribution(b.contrib) - numericContribution(a.contrib)).slice(0, 4).map(buildSponsorView),
  };
};
