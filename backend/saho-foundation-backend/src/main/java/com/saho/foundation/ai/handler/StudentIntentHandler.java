...
public class StudentIntentHandler implements IntentHandler {
    ...
    public boolean supports(String module) {
        return "students".equalsIgnoreCase(StringUtils.trimWhitespace(module));
    }

    @Override
    public ChatResponse handle(IntentDTO intent, String userMessage) {
        log.info("StudentIntentHandler received userMessage='{}', intent={}", userMessage, intent);
        ...
        try {
            ...
            log.info("Unsupported student tool for userMessage='{}': {}", userMessage, ex.getMessage());
            return responseBuilder.unsupportedStudentQuery();
        } catch (Exception ex) {
            log.error("Student tool execution failed for userMessage='{}'", userMessage, ex);
